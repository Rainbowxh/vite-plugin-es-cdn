...
export function generatePatterns(config: CdnConfig): Pattern[] { 
    ...
    for (const pattern of patterns) {
      if (pattern.regexp.test(source)) {
        ...
        if(pattern.type === 'like') {
          ...
          console.warn(`\x1b[33m[import-cdn warning] Importer: ${importer}\x1b[0m\n`)
        }
      }
    }
    return result
